...
export default {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=5346162556ac0889a345de2745e8c6e89e16bf29e30fd005c3ba68571430598b`
        );
    ...
    autoAdd(filteredCoinList) {
      ...
      setInterval(async () => {
        const f = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${currentTicker.name}&tsyms=USD&api_key=5346162556ac0889a345de2745e8c6e89e16bf29e30fd005c3ba68571430598b`
        );
        const data = await f.json();
